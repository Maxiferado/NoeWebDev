...
/* document.addEventListener('DOMContentLoaded',() =>{ //Se ejecuta hasta cuando el documento esta totalmente cargado    
    if(localStorage.getItem('carritoOverLay')){
      carritoOverLay = JSON.parse(localStorage.getItem('carritoOverLay'));// para cuando se refresca la pagina se guarde los objetos que estaban en el carrito            
    }
}); */
...
for(but of button) {    
    but.addEventListener('click', (e)=>{  
        ...
        /* localStorage.setItem('carritoOverlay', JSON.stringify(carritoOverLay)); */ 
        ...
        nuevoObjeto.innerHTML = `
            ...
            <span></span>
            <a href="carritopre.html"><button>Pagar</button></a>
        `;
